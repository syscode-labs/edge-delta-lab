...
Use HTTPS with trusted certificates. Unencrypted registry access requires both `--registry http://...` and `--allow-registry-http` (or `REGISTRY_ALLOW_HTTP=true` in hub.env). An HTTP URL alone is rejected by the packaged installer. This opt-in is only for a trusted isolated test network, not a recommended production setting. No blanket TLS-verification bypass is installed.
...
Hub uninstall removes containers/network, retaining the installation directory, manager and runtime files; `hub start` or `make hub-up` can resume. Receiver uninstall disables/removes its unit and binaries but retains `/etc/edgelab` and systemd state (which may reside under `/var/lib/private/edgelab-receiver`). A later `sudo make receiver-up` (or `receiver setup` from an extracted bundle) reinstalls using **retained configuration and trust**, not the new setup arguments. Editing hub.env or receiver.env does not update persisted configuration. This is a retained-config reinstall, not an automatic migration/upgrade workflow. To reset enrollment deliberately, uninstall first, back up and explicitly archive/remove `/etc/edgelab`; preserve verified state unless a reset is intended. Do not erase retained anti-rollback state merely to force a retry. Use stop/start for temporary shutdowns.
